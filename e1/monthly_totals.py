...
def pivot_months_pandas(data):
    ...
    df = data[['month', 'name', 'precipitation']]
    grouped_df = df.groupby(['name', 'month'])
    monthly = grouped_df.aggregate('sum').reset_index()
    monthly = monthly.pivot(index='name', columns='month', values='precipitation')
    counts = grouped_df.aggregate('count').reset_index()
...
def main():
    ...
    totals.to_csv('pandas_totals.csv')
    counts.to_csv('pandas_counts.csv')
    np.savez('pandas_monthdata.npz', totals=totals.values, counts=counts.values)
